...
public class UpdateUtil {
     ...
     * 2 应用商店
     */
    public final static int GOTOMARKETBYBROWSER = 1;
    public final static int GOTOMARKETBYAPPSTORE = 2;
    ...
    public void start(@NonNull final String apkUrl, @NonNull String title, @Nullable String message, @NonNull String startDownloadButtonText, @Nullable String cancelButtonText, @Nullable String goToMarketText, @Nullable int goToMarketType, @Nullable String goToMarketUrl, @Nullable String clickToInstall, @Nullable boolean isForced) {
        ...
        if (goToMarketText != null) {
            updateDialog.setButton(BUTTON_NEUTRAL, goToMarketText, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    // 去应用商店
                    switch (goToMarketType) {
                        case GOTOMARKETBYBROWSER: // 跳转系统自带浏览器-带链接
                            jumpBrowser(goToMarketUrl);
                            break;
                        case GOTOMARKETBYAPPSTORE: // 跳转设备内应用商店
                            launchAppDetail(contextWeakReference.get().getPackageName(), goToMarketUrl);
                            break;
                        default:
                            break;
                    }
...
//                        progressDialogDismiss();
    ...
    private File getRealFileInAndroidM(Context context, long downloadId) {
        ...
        if (downloadId != -1) {
            ...
            if (cur != null) {
                if (cur.moveToFirst()) {
                    String uriString = cur.getString(cur.getColumnIndexOrThrow(DownloadManager.COLUMN_LOCAL_URI));
    ...
    private int getProgress(long downloadId) {
        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                int downloadSoFar = cursor.getInt(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR));       //当前的下载量
